...
if [[ $OS_VER == *OSX* ]]; then
  find . -not -path '*/\.*' -not -path '*/vendor*' -not -path '*/node_modules*' -type f -name '*.php' -exec sed -i '' -e "s/\* @since\( \{1,\}\)latest/\* @since ${VERSION}/g" {} \;
else
  find . -not -path '*/\.*' -not -path '*/vendor*' -not -path '*/node_modules*' -type f -name '*.php' -exec sed -i  "s/\* @since\( \{1,\}\)latest/\* @since ${VERSION}/g" {} \;
fi
